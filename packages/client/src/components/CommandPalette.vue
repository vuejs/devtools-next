  ...
  <VueOverlay v-if="show" dim class="$ui-z-max-override backdrop-blur-2xl!">
    <div flex="~ col" h="60%" border="1 base" m-x-4 max-h-120 min-h-60 w-xl of-hidden rounded-md text-3.75>
      ...
      <div flex-auto of-auto p2 flex="~ col">
        ...
        >
          ...
          >
            ...
            <VueIcon v-if="selectedIndex === idx" icon="i-carbon-text-new-line scale-x--100" flex-none />
          </div>
        </button>
        <div v-if="!filtered.length" h-full flex items-center justify-center gap-2 text-xl>
          <VueIcon icon="carbon-search" op50 />
          <div>
      ...
      <footer border="t base" flex="~ none justify-between items-center gap-4" pointer-events-none px4 py2>
        ...
        <div text-xs flex="~ items-center gap2">
          <button px1>
            <VueIcon icon="i-carbon-text-new-line scale-x--100" scale-x--100 />
          </button>
